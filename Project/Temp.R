...
raw_online_news_dataset <- read.csv("OnlineNewsPopularity.csv", header=TRUE)
...
online_news_dataset_final <- online_news_dataset[!(online_news_dataset$data_channel_is_bus == 0 & 
                                           online_news_dataset$data_channel_is_lifestyle == 0 & 
                                           online_news_dataset$data_channel_is_tech == 0 & 
                                           online_news_dataset$data_channel_is_socmed == 0 &
                                           online_news_dataset$data_channel_is_world == 0 & 
                                           online_news_dataset$data_channel_is_entertainment == 0), ]
  

# Dropping the 1st URL column, 2nd Timedelta column and last target column
online_news_dataset_pca <- online_news_dataset_final[, c(3:60)]
...
# Checking multicollinearity using correlations

library(corrplot)
res <- cor(online_news_dataset_pca, method="pearson")
corrplot::corrplot(res, method= "color", order = "hclust", tl.col = 'black', type = 'full')

res_self <- cor(online_news_dataset_pca[27:29], method="pearson")
corrplot::corrplot(res_self, method= "color", order = "hclust", tl.col = 'black', type = 'full')

res_words <- cor(online_news_dataset_pca[43:48], method="pearson")
corrplot::corrplot(res_words, method= "color", order = "hclust", tl.col = 'black', type = 'full')

res_polarity <- cor(online_news_dataset_pca[49:54], method="pearson")
corrplot::corrplot(res_polarity, method= "color", order = "hclust", tl.col = 'black', type = 'full')

res_title <- cor(online_news_dataset_pca[55:58], method="pearson")
corrplot::corrplot(res_title, method= "color", order = "hclust", tl.col = 'black', type = 'full')
